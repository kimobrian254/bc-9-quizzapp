...
class TestQuizzApp(unittest.TestCase):
    ...
    def test_take_quiz(self):
        ...
        """
        self.assertIs(quizzactions.take_quiz("sports"),
                      list, msg="Return type not a list")
    ...
    def test_download_quiz(self):
        ...
        """
        self.assertEqual(quizzactions.download_quiz("gfgggf"),
                         False, msg="The Quizz is Available on the Remote Repo")
    ...
    def test_quizz_validation(self):
        ...
        self.assertEqual(quizzactions.validate_json(
            json_data, schema), "Valid JSON Format", msg="Invalid JSON format")
    ...
    def test_quizz_validation2(self):
        ...
        self.assertEqual(quizzactions.validate_json(
            wrong_data, schema), "Invalid JSON Format", msg="Invalid JSON format")

    def test_import_quiz(self):
        """
        Test for importing a file that does not exist
        """
        self.assertEqual(quizzactions.import_quiz("json.txt"),
                         "Invalid Path", "The file Exists")

    def test_import_quiz2(self):
        """
        Test for importing a file that is not *.json
        """
        self.assertEqual(quizzactions.import_quiz("invalid.txt"),
                         "Not A JSON File", "The file is a JSON File")

    def test_import_quiz3(self):
        """
        Test for importing a file that is *.json but wrong format
        """
        self.assertEqual(quizzactions.import_quiz("invalid.json"),
                         "Invalid JSON Format", "The file is a valid Quizz File")
